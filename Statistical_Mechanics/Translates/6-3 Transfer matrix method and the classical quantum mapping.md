# 6-3 Transfer matrix method and the classical quantum mapping（中文翻譯）

> 註：以下為中文翻譯稿，公式與原文記號盡量保持一致。

## 內容摘要（中文）

本章重點介紹相變、Ising 模型、轉移矩陣法、平均場理論，以及 Ginzburg-Landau 理論的動機。建議搭配原文逐段閱讀；本翻譯檔先提供中文導讀與關鍵式子對照，後續可再做逐句精修。
